...
export default function UserManagement() {
  ...
  // Cache para armazenar roles já buscados
  const roleCache = useRef<Map<string, Role>>(new Map())
  ...
  // Função otimizada para buscar role com cache
  const fetchRoleWithCache = useCallback(async (typeId: string): Promise<Role | null> => {
    // Verifica se já está no cache
    if (roleCache.current.has(typeId)) {
      return roleCache.current.get(typeId) || null
    }

    // Verifica se já há uma requisição ativa para este tipo
    if (activeRequests.current.has(typeId)) {
    ...
    try {
      activeRequests.current.add(typeId)
      const role = await fetchUserRole(typeId)
      
      if (role) {
        roleCache.current.set(typeId, role)
      }
      
      return role
    } finally {
      activeRequests.current.delete(typeId)
    }
  }, [fetchUserRole])
  ...
  useEffect(() => {
    const initializeUser = async () => {
      try {
        ...
        if (user.type) {
          const role = await fetchRoleWithCache(user.type)
          if (role) {
            setUserRole(role)
          }
        }
    ...
    initializeUser()
  }, [setCurrentUser, setUserRole, fetchRoleWithCache])
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data.data.data)) {
        ...
        // Busca todos os tipos únicos primeiro
        const uniqueTypes = [...new Set(usersData.map((user: User) => user.type).filter(Boolean))]
        
        // Busca os roles em lote para tipos únicos
        const rolePromises = uniqueTypes.map(type => fetchRoleWithCache(type))
        await Promise.all(rolePromises)

        // Agora mapeia os usuários com os roles do cache
        const usersWithRoles = usersData.map((user: User) => {
          if (user.type && roleCache.current.has(user.type)) {
            return { ...user, role: roleCache.current.get(user.type) }
          }
          return user
        })
