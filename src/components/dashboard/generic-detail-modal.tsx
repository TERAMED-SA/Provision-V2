          ...
          <div className="space-y-1 lg:space-y-2">
            ...
            <div className="p-3 lg:p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                <div>
                  <p className="text-xs lg:text-sm font-medium text-muted-foreground">Data</p>
                  <p className="text-xs lg:text-sm">{notification.createdAt} {notification.createdAtTime}</p>
                </div>
                <div>
                  <p className="text-xs lg:text-sm font-medium text-muted-foreground">Supervisor</p>
                  <p className="text-xs lg:text-sm">{notification.supervisorName || 'Não atribuído'}</p>
                </div>
                <div>
                  {isOccurrence && occurrenceData?.priority && (
                    <>
                      <p className="text-xs lg:text-sm font-medium text-muted-foreground">Prioridade</p>
                      <Badge className={cn('text-xs', priorityColor)}>
                        {getPriorityLabel ? getPriorityLabel(occurrenceData.priority) : occurrenceData.priority}
                      </Badge>
                    </>
                  )}
                  {!isOccurrence && supervisionData?.costCenter && (
                    <>
                      <p className="text-xs lg:text-sm font-medium text-muted-foreground">Centro de Custo</p>
                      <p className="text-xs lg:text-sm font-medium">{supervisionData.costCenter}</p>
                    </>
                  )}
                </div>
              </div>
              <div className="mt-2">
                <p className="text-xs lg:text-sm font-medium text-muted-foreground">Site</p>
                <p className="text-sm font-medium break-words">{notification.siteName}</p>
              </div>
            </div>

            {/* Details */}
            <div className="p-3 lg:p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
              <p className="text-xs lg:text-sm font-medium text-muted-foreground mb-2">Detalhes</p>
                ...
                {notification.workerInformation && notification.workerInformation.length > 0 ? (
                      ...
                      <div key={index} className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                        <div className="flex items-center justify-between mb-2 gap-2">
                          <div className="flex items-center gap-2 min-w-0 flex-1">
                            ...
                            <h4 className="text-xs lg:text-sm font-medium truncate">{worker.name}</h4>
                          </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          <div className="min-w-0">
                            ...
                            <p className="text-xs lg:text-sm break-words">{worker.state}</p>
                          </div>
                          {worker.obs && (
                            <div className="min-w-0">
                              <p className="text-xs font-medium text-muted-foreground">Observações</p>
                              <p className="text-xs break-words">{worker.obs}</p>
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="text-center py-4 lg:py-6 text-muted-foreground bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                    ...
                    <p className="text-xs lg:text-sm">
                      Nenhum trabalhador registado nesta {isOccurrence ? 'ocorrência' : 'supervisão'}.
                    </p>
                  </div>
                )}
              </TabsContent>
                ...
                {notification.equipment && notification.equipment.length > 0 ? (
                  <div className="w-full max-h-[250px] lg:max-h-[300px] overflow-y-auto border rounded-sm">
                    <Table className="w-full">
                      <TableHeader>
                        <TableRow>
                          <TableHead className="p-2 text-xs lg:text-sm">Equipamento</TableHead>
                          <TableHead className="p-2 text-xs lg:text-sm">Nº de Série</TableHead>
                          <TableHead className="p-2 text-xs lg:text-sm">Estado</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {notification.equipment.map((equip, index) => (
                          <TableRow key={index}>
                            <TableCell className="p-2 text-xs lg:text-sm break-words">
                              {equip.name}
                            </TableCell>
                            <TableCell className="p-2 text-xs lg:text-sm break-all">
                              {equip.serialNumber}
                            </TableCell>
                            <TableCell className="p-2 text-xs lg:text-sm break-words">
                              {equip.state}
                  ...
                  <div className="text-center py-4 lg:py-6 text-muted-foreground bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                    <Info className="h-5 w-5 lg:h-6 lg:w-6 mx-auto mb-2" />
                    <p className="text-xs lg:text-sm">
                      Nenhum equipamento registado nesta {isOccurrence ? 'ocorrência' : 'supervisão'}.
                    </p>
                  </div>
                )}
              </TabsContent>
